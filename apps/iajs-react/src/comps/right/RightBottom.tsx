...
function RightBottom() {
  const [currentObjects, setCurrentObjects] = useState<Record<string, any>[] | undefined>(undefined)

  listenKeys($iajsStore, ['currentObjects'], (value, oldValue, changed) => {
    setCurrentObjects(value.currentObjects)
  })
  ...
  const assembly = (item: TreeNode) => {
    ...
    return (
      <div
        key={item.id}
        className={`hs-accordion active ${item.lvl > 0 ? 'pl-5' : ''}`}
        role="treeitem"
        aria-expanded="true"
        id={item.id}
      ...
      >
        <div
          className="hs-accordion-heading flex items-center justify-between hover:bg-gray-100 cursor-pointer"
          onClick={(e) => {
            if (item.lvl > 0) return
          }}
        >
          <button
            className={`${!isDir ? 'invisible' : ''} hs-accordion-toggle flex-none`}
            aria-expanded="true"
          ...
          <div className="flex-1 grow" onClick={() => handleNodeClick(item.id)}>
            {item.name}
          </div>
          <div className={`${item.lvl > 0 ? 'hidden' : ''} flex-none w-9 flex justify-start items-center`}>
            <button className="" onClick={() => handleNodeEvented(item.id, item.evt)}>
              {item.evt && (
                <svg
                  className="shrink-0 size-4 text-gray-200 hover:text-gray-700"
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
              ...
              {!item.evt && (
                <svg
                  className="shrink-0 size-4 text-gray-700 hover:text-gray-700"
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
            ...
            <button className="" onClick={() => handleNodeVisible(item.id, item.v)}>
              {item.v && (
                <svg
                  className="shrink-0 size-4 text-gray-200 hover:text-gray-700"
                  xmlns="http://www.w3.org/2000/svg"
              ...
              if (subItem.children) {
                ...
                return (
                  <div
                    ...
                    role="treeitem"
                    className="pl-9 hover:bg-gray-100 cursor-pointer flex items-center justify-between"
                    data-hs-tree-view-item={`{ "value": "${subItem.name}" }`}
                  >
                    <div className="flex-1 grow">{subItem.name}</div>
                    <div className={`${subItem.lvl > 0 ? 'hidden' : ''} flex-none w-9 flex justify-start items-center`}>
                      <button className="" onClick={() => handleNodeEvented(subItem.id, subItem.evt)}>
                        {subItem.evt && (
                          <svg
                            className="shrink-0 size-4 text-gray-200 hover:text-gray-700"
                            xmlns="http://www.w3.org/2000/svg"
                      ...
                      <button className="" onClick={() => handleNodeVisible(subItem.id, subItem.v)}>
                        {subItem.v && (
                          <svg
                            className="shrink-0 size-4 text-gray-200 hover:text-gray-700"
                            xmlns="http://www.w3.org/2000/svg"
  ...
  return (
    <div
      ...
      className="bg-white h-full w-full grow rounded p-1 dark:bg-neutral-900 text-sm"
      data-hs-tree-view
    >
      {currentObjects &&
        currentObjects.map((item: any) => {
