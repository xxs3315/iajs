...
export class GifDecoder implements Decoder {
   ...
   */
  private static getPrefixChar(prefix: Uint32Array, code: number, clearCode: number): number {
    let c = code
    let i = 0
   ...
   */
  private static updateImage(image: MemoryImage, y: number, colorMap: GifColorMap | undefined, line: Uint8Array): void {
  ...
  private readGraphicsControlExt(input: InputBuffer<Uint8Array>): void {
    ...
    if (recordType === GifDecoder._imageDescRecordType) {
      ...
      if (this._transparentFlag !== 0) {
        if (gifImage.colorMap === undefined && this._info!.globalColorMap !== undefined) {
    ...
    while (i < lineLen) {
      ...
      if (this._currentCode === this._clearCode) {
        ...
        if (this._currentCode < this._clearCode) {
          ...
          if (j >= GifDecoder._lzMaxCode || currentPrefix > GifDecoder._lzMaxCode) {
            return false
          }
  ...
  private decompressInput(): number | undefined {
    ...
    const code: number = this._currentShiftDWord & GifDecoder._codeMasks[this._runningBits]
  ...
  private decodeImage(gifImage: GifImageDesc): MemoryImage | undefined {
    ...
    if (gifImage.x + width > this._info.width || gifImage.y + height > this._info.height) {
      return undefined
    }

    const colorMap = gifImage.colorMap !== undefined ? gifImage.colorMap! : this._info.globalColorMap!
  ...
  public decode(opt: DecoderDecodeOptions): MemoryImage | undefined {
    ...
    for (let i = 0; i < this._info.numFrames; ++i) {
      ...
      const colorMap = frame.colorMap !== undefined ? frame.colorMap : this._info.globalColorMap!
      ...
      if (frame.disposal === 2) {
        ...
        if (frame.x !== 0 || frame.y !== 0 || frame.width !== lastImage.width || frame.height !== lastImage.height) {
