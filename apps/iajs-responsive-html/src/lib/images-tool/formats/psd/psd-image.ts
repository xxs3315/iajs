...
export class PsdImage implements DecodeInfo {
   ...
   * @returns {number} The blended value.
   */
  private static blendOverlay(a: number, b: number, aAlpha: number, bAlpha: number): number {
    const x = a / 255
    const y = b / 255
    const aa = aAlpha / 255
    const ba = bAlpha / 255
  ...
  private static blendPinLight(bottom: number, top: number): number {
    return top < 128 ? this.blendDarken(bottom, 2 * top) : this.blendLighten(bottom, 2 * (top - 128))
  }
