...
export class PsdChannel {
   ...
   */
  private readLineLengths(input: InputBuffer<Uint8Array>, height: number): Uint16Array {
   ...
   */
  private readPlaneUncompressed(input: InputBuffer<Uint8Array>, width: number, height: number, bitDepth: number): void {
   ...
   */
  private decodeRLE(src: InputBuffer<Uint8Array>, dst: Uint8Array, dstIndex: number): void {
  ...
  public readPlane(opt: ReadPlaneOptions): void {
    ...
    switch (compression) {
      ...
      case PsdChannel.compressRle: {
        const lineLengths = opt.lineLengths ?? this.readLineLengths(opt.input, opt.height)
