...
export class PsdLayer {
  ...
  public get type(): number {
    if (this._additionalData.has(PsdLayerSectionDivider.tagName)) {
      const section = this._additionalData.get(PsdLayerSectionDivider.tagName) as PsdLayerSectionDivider
      return section.type
    }
  ...
  constructor(input: InputBuffer<Uint8Array>) {
    ...
    if (len > 0) {
      ...
      while (!extra.isEOS) {
        ...
        // Layer effects data
        if (tag === 'lrFX') {
          const fxData = this._additionalData.get('lrFX') as PsdLayerAdditionalData
