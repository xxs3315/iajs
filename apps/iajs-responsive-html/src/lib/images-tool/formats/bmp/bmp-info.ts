...
export class BmpInfo implements DecodeInfo {
  ...
  constructor(p: InputBuffer<Uint8Array>, header?: BmpFileHeader) {
    ...
    if (
      ...
      if (this._headerSize > 40 || this._compression === BmpCompressionMode.alphaBitfields) {
  ...
  public decodePixel(
    ...
    if (this._compression === BmpCompressionMode.bitfields && this._bitsPerPixel === 32) {
      ...
      pixel(r, g, b, a)
      return
    } else if (this._bitsPerPixel === 32 && this._compression === BmpCompressionMode.none) {
      const b = input.read()
      const g = input.read()
      const r = input.read()
      const a = input.read()
