...
export class PngEncoder implements Encoder {
   ...
   */
  private static writeChunk(out: OutputBuffer, type: string, chunk: Uint8Array): void {
   ...
   */
  private static write(bpc: number, row: Uint8Array, ri: number, out: Uint8Array, oi: number): number {
    let _bpc = bpc
    let _oi = oi
    _bpc--
    while (_bpc >= 0) {
   ...
   * @returns {number} The new output index.
   */
  private static filterSub(row: Uint8Array, bpc: number, bpp: number, out: Uint8Array, oi: number): number {
    let _oi = oi
   ...
   * @returns {number} The new output index.
   */
  private static filterUp(row: Uint8Array, bpc: number, out: Uint8Array, oi: number, prevRow?: Uint8Array): number {
    let _oi = oi
   ...
   * @returns {number} The new output index.
   */
  private static filterNone(rowBytes: Uint8Array, bpc: number, out: Uint8Array, oi: number): number {
    let _oi = oi
  ...
  private filter(image: MemoryImage, out: Uint8Array): void {
    ...
    for (let y = 0; y < image.height; ++y) {
      const rowBytes = buffer !== undefined ? new Uint8Array(buffer, rowOffset, rowStride) : new Uint8Array()
      rowOffset += rowStride
  ...
  public addFrame(image: MemoryImage): void {
    ...
    if (
      ...
      (_image.bitsPerChannel < 8 && !_image.hasPalette && _image.numChannels > 1)
    ) {
