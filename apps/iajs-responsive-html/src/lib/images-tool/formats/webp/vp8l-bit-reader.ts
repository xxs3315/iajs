...
export class VP8LBitReader {
  ...
  private shiftBytes(): void {
    while (this._bitPos >= 8 && !this._input.isEOS) {
      ...
      this._buffer[0] = (this._buffer[0] >>> 8) + (this._buffer[1] & 0xff) * 0x1000000
      this._buffer[1] >>>= 8
  ...
  public prefetchBits(): number {
    ...
    if (this._bitPos < 32) {
      b2 =
        ...
        (this._buffer[1] & VP8LBitReader.bitMask[this._bitPos]) * (VP8LBitReader.bitMask[32 - this._bitPos] + 1)
