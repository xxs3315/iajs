...
export class HuffmanTree {
  ...
  private reverseBitsShort(bits: number, numBits: number): number {
    const v = (HuffmanTree.reversedBits[bits & 0xf] << 4) | HuffmanTree.reversedBits[bits >>> 4]
    return v >>> (8 - numBits)
  }
   ...
   */
  private huffmanCodeLengthsToCodes(codeLengths: Int32Array, codeLengthsSize: number, huffCodes: Int32Array): boolean {
    let symbol: number = 0
   ...
   */
  public buildImplicit(codeLengths: Int32Array, codeLengthsSize: number): boolean {
    let numSymbols = 0
    let rootSymbol = 0
