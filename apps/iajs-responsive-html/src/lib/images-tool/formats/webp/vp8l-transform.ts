...
export class VP8LTransform {
    ...
    switch (this._type) {
      ...
      case VP8LImageTransformType.colorIndexing:
        if (rowsIn === rowsOut && this._bits > 0) {
          ...
          const inStride = (rowEnd - rowStart) * VP8LInternal.subSampleSize(this._xsize, this._bits)
    ...
    if (bitsPerPixel < 8) {
      ...
      for (let y = yStart; y < yEnd; ++y) {
        ...
        for (let x = 0; x < width; ++x) {
          ...
          const p = VP8LTransform.getAlphaValue(colorMap![packedPixels & bitMask])
          dst.set(0, p)
          dst.offset++
    ...
    if (bitsPerPixel < 8) {
      ...
      for (let y = yStart; y < yEnd; ++y) {
        ...
        for (let x = 0; x < width; ++x) {
          ...
          outData[_dst++] = VP8LTransform.getARGBValue(colorMap![packedPixels & bitMask])
          packedPixels >>>= bitsPerPixel
        }
      }
    } else {
      for (let y = yStart; y < yEnd; ++y) {
        for (let x = 0; x < width; ++x) {
          outData[_dst++] = VP8LTransform.getARGBValue(colorMap![VP8LTransform.getARGBIndex(inData[_src++])])
        }
      }
    }
  }
   ...
   */
  public colorSpaceInverseTransform(yStart: number, yEnd: number, outData: Uint32Array, data: number): void {
    let _data = data
    const width = this._xsize
   ...
   * @param {number} data - Data index.
   */
  public predictorInverseTransform(yStart: number, yEnd: number, outData: Uint32Array, data: number): void {
    let _data = data
    let _yStart = yStart
    const width = this._xsize
    if (_yStart === 0) {
      ...
      for (let x = 1; x < width; ++x) {
        const pred1 = VP8LTransform.predictor1(outData, outData[_data + x - 1], 0)
    ...
    while (y < yEnd) {
      const pred2 = VP8LTransform.predictor2(outData, outData[_data - 1], _data - width)
   ...
   */
  public addGreenToBlueAndRed(pixels: Uint32Array, data: number, dataEnd: number): void {
    let _data = data
    while (_data < dataEnd) {
   ...
   * @returns {number} The average value.
   */
  private static average4(a0: number, a1: number, a2: number, a3: number): number {
   ...
   */
  private static addSubtractComponentFull(a: number, b: number, c: number): number {
   ...
   * @returns {number} The result.
   */
  private static clampedAddSubtractFull(c0: number, c1: number, c2: number): number {
   ...
   */
  private static clampedAddSubtractHalf(c0: number, c1: number, c2: number): number {
   ...
   */
  private static predictor0(_pixels: Uint32Array, _left: number, _top: number): number {
   ...
   */
  private static predictor1(_pixels: Uint32Array, left: number, _top: number): number {
    return left
  }
   ...
   */
  private static predictor2(pixels: Uint32Array, _left: number, top: number): number {
    return pixels[top]
  }
   ...
   * @returns {number} The predicted pixel value.
   */
  private static predictor3(pixels: Uint32Array, _left: number, top: number): number {
    return pixels[top + 1]
  }
   ...
   * @returns {number} The predicted pixel value.
   */
  private static predictor4(pixels: Uint32Array, _left: number, top: number): number {
    return pixels[top - 1]
  }
   ...
   * @returns {number} The predicted pixel value.
   */
  private static predictor5(pixels: Uint32Array, left: number, top: number): number {
   ...
   * @returns {number} The predicted pixel value.
   */
  private static predictor6(pixels: Uint32Array, left: number, top: number): number {
   ...
   * @returns {number} The predicted pixel value.
   */
  private static predictor7(pixels: Uint32Array, left: number, top: number): number {
   ...
   * @returns {number} The predicted pixel value.
   */
  private static predictor8(pixels: Uint32Array, _left: number, top: number): number {
   ...
   * @returns {number} The predicted pixel value.
   */
  private static predictor9(pixels: Uint32Array, _left: number, top: number): number {
   ...
   * @returns {number} The predicted pixel value.
   */
  private static predictor10(pixels: Uint32Array, left: number, top: number): number {
    return VP8LTransform.average4(left, pixels[top - 1], pixels[top], pixels[top + 1])
  }
   ...
   * @returns {number} The predicted pixel value.
   */
  private static predictor11(pixels: Uint32Array, left: number, top: number): number {
   ...
   * @returns {number} The predicted pixel value.
   */
  private static predictor12(pixels: Uint32Array, left: number, top: number): number {
    return VP8LTransform.clampedAddSubtractFull(left, pixels[top], pixels[top - 1])
  }
   ...
   * @returns {number} The predicted pixel value.
   */
  private static predictor13(pixels: Uint32Array, left: number, top: number): number {
    return VP8LTransform.clampedAddSubtractHalf(left, pixels[top], pixels[top - 1])
  }
