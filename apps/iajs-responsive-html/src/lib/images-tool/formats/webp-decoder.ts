...
export class WebPDecoder implements Decoder {
   ...
   */
  private decodeFrameInternal(input: InputBuffer<Uint8Array>, frameIndex: number = 0): MemoryImage | undefined {
   ...
   */
  private getInfo(input: InputBuffer<Uint8Array>, webp: WebPInfoInternal): boolean {
    while (!input.isEOS) {
   ...
   * @returns {boolean} True if successful, false otherwise.
   */
  private getVp8xInfo(input: InputBuffer<Uint8Array>, webp?: WebPInfo): boolean {
    const b = input.read()
    if ((b & 0xc0) !== 0) {
      return false
    }
   ...
   * @returns {boolean} True if successful, false otherwise.
   */
  private getAnimFrameInfo(input: InputBuffer<Uint8Array>, webp: WebPInfo, size: number): boolean {
