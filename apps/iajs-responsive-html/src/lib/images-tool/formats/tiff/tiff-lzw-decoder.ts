...
export class LzwDecoder {
  ...
  private getNextCode(): number {
    ...
    while (this._nextBits < this._bitsToGet) {
      ...
      this._nextData = ((this._nextData << 8) + this._data[this._bytePointer++]) & 0xffffffff
      this._nextBits += 8
    }

    this._nextBits -= this._bitsToGet
    const code = (this._nextData >>> this._nextBits) & LzwDecoder._andTable[this._bitsToGet - 9]
