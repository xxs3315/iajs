...
export class TiffImage {
  ...
  constructor(p: InputBuffer<Uint8Array>) {
    ...
    if (this._colorMap !== undefined && this._photometricType === TiffPhotometricType.palette) {
    ...
    switch (this._photometricType) {
      ...
      case TiffPhotometricType.palette:
        if (
          ...
          (this._bitsPerSample === 4 || this._bitsPerSample === 8 || this._bitsPerSample === 16)
        ) {
      ...
      case TiffPhotometricType.yCbCr:
        if (this._compression === TiffCompression.jpeg && this._bitsPerSample === 8 && this._samplesPerPixel === 3) {
          this._imageType = TiffImageType.rgb
        } else {
   ...
   */
  private decodeBilevelTile(p: InputBuffer<Uint8Array>, image: MemoryImage, tileX: number, tileY: number): void {
    ...
    if (this._compression === TiffCompression.packBits) {
      ...
      if (this._tileWidth % 8 === 0) {
        ...
        bytesInThisTile = (Math.trunc(this._tileWidth / 8) + 1) * this._tileHeight
      }
      ...
      decoder.decode(InputBuffer.from(p, 0, byteCount), byteData.buffer as Uint8Array)
   ...
   */
  private decodeTile(p: InputBuffer<Uint8Array>, image: MemoryImage, tileX: number, tileY: number): void {
        ...
        if (this._predictor === 2) {
          ...
          for (let j = 0; j < this._tileHeight; j++) {
            ...
            for (let i = this._samplesPerPixel; i < len; i++) {
              byteData.set(count, byteData.get(count) + byteData.get(count - this._samplesPerPixel))
              count++
            }
          }
        }
          ...
          if (this._samplesPerPixel === 1) {
            if (this._sampleFormat === TiffFormat.float) {
              ...
              if (this._bitsPerSample === 8) {
                sample = this._sampleFormat === TiffFormat.int ? byteData.readInt8() : byteData.read()
              } else if (this._bitsPerSample === 16) {
                sample = this._sampleFormat === TiffFormat.int ? byteData.readInt16() : byteData.readUint16()
              } else if (this._bitsPerSample === 32) {
                sample = this._sampleFormat === TiffFormat.int ? byteData.readInt32() : byteData.readUint32()
              }
            ...
            let alpha = 0
            if (this._bitsPerSample === 8) {
              gray = this._sampleFormat === TiffFormat.int ? byteData.readInt8() : byteData.read()
              alpha = this._sampleFormat === TiffFormat.int ? byteData.readInt8() : byteData.read()
            } else if (this._bitsPerSample === 16) {
              gray = this._sampleFormat === TiffFormat.int ? byteData.readInt16() : byteData.readUint16()
              alpha = this._sampleFormat === TiffFormat.int ? byteData.readInt16() : byteData.readUint16()
            } else if (this._bitsPerSample === 32) {
              gray = this._sampleFormat === TiffFormat.int ? byteData.readInt32() : byteData.readUint32()
              alpha = this._sampleFormat === TiffFormat.int ? byteData.readInt32() : byteData.readUint32()
            }
            ...
            if (this._sampleFormat === TiffFormat.float) {
              ...
              let b = 0
              if (this._bitsPerSample === 8) {
                r = this._sampleFormat === TiffFormat.int ? byteData.readInt8() : byteData.read()
                g = this._sampleFormat === TiffFormat.int ? byteData.readInt8() : byteData.read()
                b = this._sampleFormat === TiffFormat.int ? byteData.readInt8() : byteData.read()
              } else if (this._bitsPerSample === 16) {
                r = this._sampleFormat === TiffFormat.int ? byteData.readInt16() : byteData.readUint16()
                g = this._sampleFormat === TiffFormat.int ? byteData.readInt16() : byteData.readUint16()
                b = this._sampleFormat === TiffFormat.int ? byteData.readInt16() : byteData.readUint16()
              } else if (this._bitsPerSample === 32) {
                r = this._sampleFormat === TiffFormat.int ? byteData.readInt32() : byteData.readUint32()
                g = this._sampleFormat === TiffFormat.int ? byteData.readInt32() : byteData.readUint32()
                b = this._sampleFormat === TiffFormat.int ? byteData.readInt32() : byteData.readUint32()
              }
            ...
            if (this._sampleFormat === TiffFormat.float) {
              ...
              if (this._bitsPerSample === 8) {
                ...
                g = this._sampleFormat === TiffFormat.int ? byteData.readInt8() : byteData.read()
                b = this._sampleFormat === TiffFormat.int ? byteData.readInt8() : byteData.read()
                a = this._sampleFormat === TiffFormat.int ? byteData.readInt8() : byteData.read()
              } else if (this._bitsPerSample === 16) {
                r = this._sampleFormat === TiffFormat.int ? byteData.readInt16() : byteData.readUint16()
                g = this._sampleFormat === TiffFormat.int ? byteData.readInt16() : byteData.readUint16()
                b = this._sampleFormat === TiffFormat.int ? byteData.readInt16() : byteData.readUint16()
                a = this._sampleFormat === TiffFormat.int ? byteData.readInt16() : byteData.readUint16()
              } else if (this._bitsPerSample === 32) {
                r = this._sampleFormat === TiffFormat.int ? byteData.readInt32() : byteData.readUint32()
                g = this._sampleFormat === TiffFormat.int ? byteData.readInt32() : byteData.readUint32()
                b = this._sampleFormat === TiffFormat.int ? byteData.readInt32() : byteData.readUint32()
                a = this._sampleFormat === TiffFormat.int ? byteData.readInt32() : byteData.readUint32()
              }
   ...
   */
  private decodePackBits(data: InputBuffer<Uint8Array>, arraySize: number, dst: Uint8Array): void {
    let srcCount = 0
    let dstCount = 0
                      ...
                      : isInt && this._bitsPerSample === 32
                        ...
                        : this._bitsPerSample === 16
                          ...
                          : this._bitsPerSample === 32
                            ? Format.uint32
                            : Format.uint8
    const hasPalette = this._colorMap !== undefined && this._photometricType === TiffPhotometricType.palette
