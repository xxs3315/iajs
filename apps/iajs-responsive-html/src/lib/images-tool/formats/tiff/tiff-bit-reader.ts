...
export class TiffBitReader {
  ...
  public readBits(numBits: number): number {
    ...
    while (nBits > this._bitPosition) {
      value = (value << this._bitPosition) + (this._bitBuffer & TiffBitReader._bitMask[this._bitPosition])
    ...
    if (nBits > 0) {
      ...
      value = (value << nBits) + ((this._bitBuffer >>> (this._bitPosition - nBits)) & TiffBitReader._bitMask[nBits])
