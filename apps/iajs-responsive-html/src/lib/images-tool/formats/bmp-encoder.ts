...
export class BmpEncoder implements Encoder {
  ...
  public encode(opt: EncoderEncodeOptions): Uint8Array {
    ...
    const rowPadding = rowPaddingSize > 0 ? new Uint8Array(rowPaddingSize).fill(0xff) : undefined
    ...
    if (bpp === 1 || bpp === 2 || bpp === 4 || bpp === 8) {
      if (palette !== undefined) {
        const l = palette.numColors > implicitPaletteSize ? implicitPaletteSize : palette.numColors
        let pi: number = 0
        for (pi = 0; pi < l; ++pi) {
    ...
    if (bpp === 1 || bpp === 2 || bpp === 4 || bpp === 8) {
      ...
      for (let y = 0; y < h; ++y) {
        const bytes = image.buffer !== undefined ? new Uint8Array(image.buffer, offset, imageStride) : new Uint8Array()
