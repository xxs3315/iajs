...
export class PngDecoder implements Decoder {
   ...
   */
  private static unfilter(filterType: PngFilterType, bpp: number, row: Uint8Array, prevRow?: Uint8Array): void {
    ...
    while (true) {
      ...
      switch (chunkType) {
        case 'tEXt':
          {
            ...
            for (let i = 0; i < l; ++i) {
              if (txtData[i] === 0) {
                const key = StringUtils.latin1Decoder.decode(ArrayUtils.copyUint8(txtData, 0, i))
                const text = StringUtils.latin1Decoder.decode(ArrayUtils.copyUint8(txtData, i + 1))
        ...
        case 'pHYs': {
          ...
          this._info.pixelDimensions = new PngPhysicalPixelDimensions(x, y, unit)
          // CRC
          this._input.skip(4)
          break
        }
        ...
        case 'tRNS': {
          this._info.transparency = this._input.readRange(chunkSize).toUint8Array()
          const crc = this._input.readUint32()
        ...
        case 'bKGD': {
          if (this._info.colorType === PngColorType.indexed) {
            ...
            if (this._info.transparency !== undefined) {
              const isTransparent = this._info.transparency.includes(paletteIndex)
              this._info.backgroundColor = new ColorRgba8(r, g, b, isTransparent ? 0 : 255)
            } else {
            ...
            chunkSize -= 2
          } else if (this._info.colorType === PngColorType.rgb || this._info.colorType === PngColorType.rgba) {
            /* Const r: number = */
            this._input.readUint16()
            /* Const g: number = */
            this._input.readUint16()
  ...
  public decodeFrame(frameIndex: number): MemoryImage | undefined {
    ...
    if (
      ...
      // 8-bit palette value.
      const to8bit = this._info.bits === 1 ? 255 : this._info.bits === 2 ? 85 : this._info.bits === 4 ? 17 : 1
    ...
    if (this._info.colorType === PngColorType.rgb && this._info.transparency !== undefined) {
      numChannels = 4
    }
    ...
    if (this._info.iccpData !== undefined) {
      opt.iccProfile = new IccProfile(this._info.iccpName, IccProfileCompression.deflate, this._info.iccpData)
    }
  ...
  public decode(opt: DecoderDecodeOptions): MemoryImage | undefined {
    ...
    for (let i = 0; i < this._info.numFrames; ++i) {
      ...
      lastImage = Draw.compositeImage({
        ...
        dstX: frame.xOffset,
        dstY: frame.yOffset,
        blend: frame.blend === PngBlendMode.over ? BlendMode.alpha : BlendMode.direct,
      })
