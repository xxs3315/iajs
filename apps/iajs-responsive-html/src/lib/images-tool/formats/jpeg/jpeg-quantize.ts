...
export abstract class JpegQuantize {
  ...
  public static getImageFromJpeg(jpeg: JpegData): MemoryImage {
    const orientation = jpeg.exifData.imageIfd.hasOrientation ? jpeg.exifData.imageIfd.orientation! : 0
          ...
          for (let y = 0; y < jpeg.height!; y++) {
            ...
            for (let x = 0; x < jpeg.width!; x++) {
              ...
              if (!colorTransform) {
                ...
                cm = 255 - MathUtils.clampInt255(cy - 0.3441363 * (cb - 128) - 0.71413636 * (cr - 128))
