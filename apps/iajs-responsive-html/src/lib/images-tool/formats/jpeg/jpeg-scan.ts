...
export class JpegScan {
   ...
   */
  private decodeHuffman(tree: Array<HuffmanNode | undefined>): number | undefined {
   ...
   */
  private decodeMcu(component: JpegComponent, decodeFn: DecodeFunction, mcu: number, row: number, col: number): void {
   ...
   * @param {number} mcu - The MCU index.
   */
  private decodeBlock(component: JpegComponent, decodeFn: DecodeFunction, mcu: number): void {
  ...
  public decode(): void {
    ...
    if (this._progressive) {
      if (this._spectralStart === 0) {
        decodeFn = this._successivePrev === 0 ? this.decodeDCFirst : this.decodeDCSuccessive
      } else {
        decodeFn = this._successivePrev === 0 ? this.decodeACFirst : this.decodeACSuccessive
      }
    } else {
    ...
    if (componentsLength === 1) {
      mcuExpected = this._components[0].blocksPerLine * this._components[0].blocksPerColumn
    } else {
