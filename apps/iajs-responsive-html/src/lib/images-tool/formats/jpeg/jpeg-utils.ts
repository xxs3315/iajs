...
export class JpegUtils {
   ...
   */
  private readExifData(block: InputBuffer<Uint8Array> | undefined): ExifData | undefined {
    if (block === undefined) {
      return undefined
    }
   ...
   */
  private readBlock(input: InputBuffer<Uint8Array>): InputBuffer<Uint8Array> | undefined {
   ...
   */
  private skipBlock(input: InputBuffer<Uint8Array>, output?: OutputBuffer): boolean {
    const length = input.readUint16()
   ...
   */
  private nextMarker(input: InputBuffer<Uint8Array>, output?: OutputBuffer): number {
    let c = 0
    if (input.isEOS) {
      return c
    }
