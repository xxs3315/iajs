...
export class PvrDecoder implements Decoder {
  ...
  private decodePvr2(data: Uint8Array): MemoryImage | undefined {
    ...
    if ((info.width * info.height * info.bitsPerPixel) / 8 > length - PvrDecoder.pvrHeaderSize) {
      return undefined
    }
   ...
   */
  private decodeRgb4bpp(width: number, height: number, data: TypedArray): MemoryImage {
            ...
            const ca = p0
              ...
              .add(
                p1
                  ...
                  .add(
                    p2.getColorRgbA().mul(factors[factorIndex][2]).add(p3.getColorRgbA().mul(factors[factorIndex][3])),
                  ),
              )
            ...
            const cb = p0
              ...
              .add(
                p1
                  ...
                  .mul(factors[factorIndex][1])
                  .add(
                    p2.getColorRgbB().mul(factors[factorIndex][2]).add(p3.getColorRgbB().mul(factors[factorIndex][3])),
                  ),
              )
   ...
   * @returns {MemoryImage} The decoded MemoryImage.
   */
  private decodeRgba4bpp(width: number, height: number, data: TypedArray): MemoryImage {
    const result = new MemoryImage({
      width: width,
      height: height,
      numChannels: 4,
   ...
   */
  public startDecode(bytes: Uint8Array): PvrAppleInfo | Pvr3Info | Pvr2Info | undefined {
