 ...
 */
export function getRowStride(width: number, numChannels: number, format: Format): number {
  ...
  switch (from) {
    ...
    case Format.float64:
      switch (to) {
        ...
        case Format.int32:
          return Math.trunc(
            value < 0 ? MathUtils.clamp(value, -1, 1) * 2147483648 : MathUtils.clamp(value, -1, 1) * 2147483647,
          )
        case Format.float16:
        case Format.float32:
        case Format.float64:
