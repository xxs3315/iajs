...
export abstract class ColorUtils {
   ...
   */
  public static rgbaToUint32(r: number, g: number, b: number, a: number): number {
    return (
   ...
   */
  public static hslToRgb(hue: number, saturation: number, lightness: number): number[] {
    if (saturation === 0) {
   ...
   */
  public static hsvToRgb(hue: number, saturation: number, brightness: number): number[] {
    if (saturation === 0) {
    ...
    switch (Math.trunc(h)) {
      case 0:
        return [Math.round(brightness * 255), Math.round(t * 255), Math.round(p * 255)]
      case 1:
        return [Math.round(q * 255), Math.round(brightness * 255), Math.round(p * 255)]
      case 2:
        return [Math.round(p * 255), Math.round(brightness * 255), Math.round(t * 255)]
      case 3:
        return [Math.round(p * 255), Math.round(q * 255), Math.round(brightness * 255)]
      case 4:
        return [Math.round(t * 255), Math.round(p * 255), Math.round(brightness * 255)]
      case 5:
        return [Math.round(brightness * 255), Math.round(p * 255), Math.round(q * 255)]
      default:
  ...
  public static labToXyz(l: number, a: number, b: number): number[] {
    ...
    return [Math.trunc(x * 95.047), Math.trunc(y * 100), Math.trunc(z * 108.883)]
  }
  ...
  public static xyzToRgb(x: number, y: number, z: number): number[] {
    ...
    return [MathUtils.clampInt255(r * 255), MathUtils.clampInt255(g * 255), MathUtils.clampInt255(b * 255)]
  }
   ...
   * @returns {number[]} An array containing the RGB values [red, green, blue], each ranging from 0 to 255.
   */
  public static cmykToRgb(c: number, m: number, y: number, k: number): number[] {
    const _c = c / 255
    const _m = m / 255
    const _y = y / 255
    const _k = k / 255
