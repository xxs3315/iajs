...
export abstract class Transform {
  ...
  private static rotate90(src: MemoryImage): MemoryImage {
    ...
    for (const frame of src.frames) {
      const dst: MemoryImage = firstFrame?.addFrame() ?? MemoryImage.fromResized(frame, frame.height, frame.width, true)
      firstFrame ??= dst
  ...
  private static rotate180(src: MemoryImage): MemoryImage {
    ...
    for (const frame of src.frames) {
      ...
      const hm1 = frame.height - 1
      const dst: MemoryImage = firstFrame?.addFrame() ?? MemoryImage.from(frame, true, true)
      firstFrame ??= dst
      for (let y = 0; y < dst.height; ++y) {
        for (let x = 0; x < dst.width; ++x) {
  ...
  public static bakeOrientation(opt: TransformOptions): MemoryImage {
    ...
    if (!opt.image.exifData.imageIfd.hasOrientation || opt.image.exifData.imageIfd.orientation === 1) {
      return bakedImage
    }
  ...
  public static copyCrop(opt: CopyCropOptions): MemoryImage {
    ...
    for (let i = 0; i < numFrames; ++i) {
      ...
      const dst: MemoryImage = firstFrame?.addFrame() ?? MemoryImage.fromResized(frame, width, height, true)
      firstFrame ??= dst
  ...
  public static copyCropCircle(opt: CopyCropCircleOptions): MemoryImage {
    ...
    let radius = opt.radius ?? Math.trunc(Math.min(image.width, image.height) / 2)
    const antialias = opt.antialias ?? true
    ...
    for (let i = 0; i < numFrames; ++i) {
      const frame = image.frames[i]
      const dst: MemoryImage = firstFrame?.addFrame() ?? MemoryImage.fromResized(frame, wh, wh, true)
      firstFrame ??= dst
  ...
  public static copyExpandCanvas(opt: CopyExpandCanvasOptions): MemoryImage {
    ...
    if ((newWidth === undefined || newHeight === undefined) && padding === undefined) {
      throw new LibError('Either new dimensions or padding must be provided.')
    } else if (newWidth !== undefined && newHeight !== undefined && padding !== undefined) {
    ...
    if (opt.toImage !== undefined && (opt.toImage.width !== newWidth || opt.toImage.height !== newHeight)) {
  ...
  public static copyRectify(opt: CopyRectifyOptions): MemoryImage {
    ...
    let firstFrame: MemoryImage | undefined = undefined
    for (const frame of src.frames) {
      const dst: MemoryImage = firstFrame?.addFrame() ?? opt.toImage ?? MemoryImage.from(frame, true)
      firstFrame ??= dst
      for (let y = 0; y < dst.height; ++y) {
        ...
        for (let x = 0; x < dst.width; ++x) {
          ...
          const srcPixelCoord = opt.topLeft
            .mul((1 - u) * (1 - v))
            .add(opt.topRight.mul(u * (1 - v)).add(opt.bottomLeft.mul((1 - u) * v).add(opt.bottomRight.mul(u * v))))
   ...
   */
  public static copyResize(opt: CopyResizeOptionsUsingWidth | CopyResizeOptionsUsingHeight): MemoryImage {
    let src = opt.image
    ...
    if (src.exifData.imageIfd.hasOrientation && src.exifData.imageIfd.orientation !== 1) {
    ...
    if (opt.width !== undefined && opt.height !== undefined && maintainAspect === true) {
      x1 = 0
      x2 = opt.width
    ...
    const height =
      opt.height === undefined || opt.height <= 0 ? Math.round(opt.width! * (src.height / src.width)) : opt.height

    const width =
      opt.width === undefined || opt.width <= 0 ? Math.round(opt.height! * (src.width / src.height)) : opt.width
   ...
   */
  public static copyResizeCropSquare(opt: CopyResizeCropSquareOptions): MemoryImage {
    const src = opt.image
    let interpolation = opt.interpolation ?? Interpolation.nearest
    const radius = opt.radius ?? 0
    const antialias = opt.antialias ?? true
    ...
    const scaleX = interpolation === Interpolation.nearest ? new Int32Array(opt.size) : undefined
    ...
    let firstFrame: MemoryImage | undefined = undefined
    for (const frame of src.frames) {
      const dst: MemoryImage = firstFrame?.addFrame() ?? MemoryImage.fromResized(frame, opt.size, opt.size, true)
      firstFrame ??= dst
  ...
  public static copyRotate(opt: CopyRotateOptions): MemoryImage {
    ...
    for (let i = 0; i < numFrames; ++i) {
      ...
      const dst: MemoryImage =
        firstFrame?.addFrame() ?? MemoryImage.fromResized(src, Math.trunc(ux + vx), Math.trunc(uy + vy), true)
      firstFrame ??= dst
  ...
  public static trim(opt: TrimOptions): MemoryImage {
    ...
    for (const frame of opt.image.frames) {
      const dst: MemoryImage = firstFrame?.addFrame() ?? MemoryImage.fromResized(frame, crop.width, crop.height, true)
      firstFrame ??= dst
