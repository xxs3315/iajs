...
export abstract class Filter {
   ...
   * @private
   */
  private static readonly _gaussianKernelCache: Map<number, SeparableKernel> = new Map<number, SeparableKernel>()
  ...
  public static adjustColor(opt: AdjustColorOptions): MemoryImage {
    ...
    const contrast = opt.contrast !== undefined ? MathUtils.clamp(opt.contrast, 0, 2) : undefined
    const saturation = opt.saturation !== undefined ? MathUtils.clamp(opt.saturation, 0, 2) : undefined
    ...
    let exposure = opt.exposure !== undefined ? MathUtils.clamp(opt.exposure, 0, 1000) : undefined
    ...
    const useBlacksWhitesMids = opt.blacks !== undefined || opt.whites !== undefined || opt.mids !== undefined
    let br = 0
    let bg = 0
    let bb = 0
    let wr = 0
    ...
    const invSaturation = saturation !== undefined ? 1 - MathUtils.clamp(saturation, 0, 2) : 0
    const invContrast = contrast !== undefined ? 1 - MathUtils.clamp(contrast, 0, 2) : 0
    ...
    for (const frame of image.frames) {
      for (const p of frame) {
        ...
        const msk = opt.mask?.getPixel(p.x, p.y).getChannelNormalized(maskChannel) ?? 1
  ...
  public static billboard(opt: BillboardOptions): MemoryImage {
    ...
    for (const frame of image.frames) {
      ...
      for (const p of frame) {
        ...
        const msk = opt.mask?.getPixel(p.x, p.y).getChannelNormalized(maskChannel)
    ...
    for (const frame of opt.image.frames) {
      for (let y = 0; y < frame.height; ++y) {
        for (let x = 0; x < frame.width; ++x) {
          ...
          let du = (height - frame.getPixel(x < frame.width - 1 ? x + 1 : x, y).r / mx) * strength
          let dv = (height - frame.getPixel(x, y < frame.height - 1 ? y + 1 : y).r / mx) * strength
   ...
   */
  public static chromaticAberration(opt: ChromaticAberrationOptions): MemoryImage {
    const image = opt.image.hasPalette
      ? opt.image.convert({
          numChannels: opt.image.numChannels,
        })
  ...
  public static colorHalftone(opt: ColorHalftone): MemoryImage {
    ...
    const pattern = (x: number, y: number, cx: number, cy: number, angle: number): number => {
  ...
  public static contrast(opt: ContrastOptions): MemoryImage {
    ...
    if (Filter._contrastCache === undefined || opt.contrast !== Filter._contrastCache.lastContrast) {
  ...
  public static copyImageChannels(opt: CopyImageChannelsOptions): MemoryImage {
    ...
    for (const frame of image.frames) {
      for (const p of frame) {
        ...
        const r = opt.red !== undefined ? fromPixel.getChannelNormalized(opt.red) : p.rNormalized
        const g = opt.green !== undefined ? fromPixel.getChannelNormalized(opt.green) : p.gNormalized
        const b = opt.blue !== undefined ? fromPixel.getChannelNormalized(opt.blue) : p.bNormalized
        const a = opt.alpha !== undefined ? fromPixel.getChannelNormalized(opt.alpha) : p.aNormalized
  ...
  public static dotScreen(opt: DotScreenOptions): MemoryImage {
    ...
    for (const frame of image.frames) {
      ...
      const pattern = (cntX: number, cntY: number, tx: number, ty: number): number => {
  ...
  public static edgeGlow(opt: EdgeGlowOptions): MemoryImage {
    ...
    for (const frame of image.frames) {
      ...
      for (const p of frame) {
        ...
        const xxR =
          t1.rNormalized + 2 * t2.rNormalized + t3.rNormalized - t7.rNormalized - 2 * t8.rNormalized - t9.rNormalized
        const xxG =
          t1.gNormalized + 2 * t2.gNormalized + t3.gNormalized - t7.gNormalized - 2 * t8.gNormalized - t9.gNormalized
        const xxB =
          t1.bNormalized + 2 * t2.bNormalized + t3.bNormalized - t7.bNormalized - 2 * t8.bNormalized - t9.bNormalized

        const yyR =
          t1.rNormalized - t3.rNormalized + 2 * t4.rNormalized - 2 * t6.rNormalized + t7.rNormalized - t9.rNormalized
        const yyG =
          t1.gNormalized - t3.gNormalized + 2 * t4.gNormalized - 2 * t6.gNormalized + t7.gNormalized - t9.gNormalized
        const yyB =
          t1.bNormalized - t3.bNormalized + 2 * t4.bNormalized - 2 * t6.bNormalized + t7.bNormalized - t9.bNormalized
  ...
  public static hdrToLdr(opt: HdrToLdrOptions): MemoryImage {
    ...
    const m = opt.exposure !== undefined ? Math.pow(2, MathUtils.clamp(opt.exposure + 2.47393, -20, 20)) : 1
   ...
   */
  public static luminanceThreshold(opt: LuminanceThresholdOptions): MemoryImage {
    const image = opt.image.hasPalette
      ? opt.image.convert({
          numChannels: opt.image.numChannels,
        })
    ...
    for (const frame of image.frames) {
      for (const p of frame) {
        const y = 0.3 * p.rNormalized + 0.59 * p.gNormalized + 0.11 * p.bNormalized
        if (outputColor) {
    ...
    for (const frame of image.frames) {
      switch (type) {
        case NoiseType.gaussian:
          for (const p of frame) {
            const r = MathUtils.clamp(p.r + nSigma * RandomUtils.grand(), 0, p.maxChannelValue)
            const g = MathUtils.clamp(p.g + nSigma * RandomUtils.grand(), 0, p.maxChannelValue)
            const b = MathUtils.clamp(p.b + nSigma * RandomUtils.grand(), 0, p.maxChannelValue)
            const a = p.a
            const msk = opt.mask?.getPixel(p.x, p.y).getChannelNormalized(maskChannel)
            if (msk === undefined) {
              p.setRgba(r, g, b, a)
        ...
        case NoiseType.uniform:
          for (const p of frame) {
            const r = MathUtils.clamp(p.r + nSigma * RandomUtils.crand(), 0, p.maxChannelValue)
            const g = MathUtils.clamp(p.g + nSigma * RandomUtils.crand(), 0, p.maxChannelValue)
            const b = MathUtils.clamp(p.b + nSigma * RandomUtils.crand(), 0, p.maxChannelValue)
            const a = p.a
            const msk = opt.mask?.getPixel(p.x, p.y).getChannelNormalized(maskChannel)
            if (msk === undefined) {
              p.setRgba(r, g, b, a)
        ...
        case NoiseType.saltAndPepper:
          ...
          for (const p of frame) {
            if (Math.random() * 100 < nSigma) {
              const r = MathUtils.clamp(Math.random() < 0.5 ? max : min, 0, p.maxChannelValue)
              const g = MathUtils.clamp(Math.random() < 0.5 ? max : min, 0, p.maxChannelValue)
              const b = MathUtils.clamp(Math.random() < 0.5 ? max : min, 0, p.maxChannelValue)
              const a = p.a
              const msk = opt.mask?.getPixel(p.x, p.y).getChannelNormalized(maskChannel)
              if (msk === undefined) {
                p.setRgba(r, g, b, a)
        ...
        case NoiseType.poisson:
          for (const p of frame) {
            const r = MathUtils.clamp(RandomUtils.prand(p.r), 0, p.maxChannelValue)
            const g = MathUtils.clamp(RandomUtils.prand(p.g), 0, p.maxChannelValue)
            const b = MathUtils.clamp(RandomUtils.prand(p.b), 0, p.maxChannelValue)
            const a = p.a
            const msk = opt.mask?.getPixel(p.x, p.y).getChannelNormalized(maskChannel)
            if (msk === undefined) {
              p.setRgba(r, g, b, a)
        ...
        case PixelateMode.average:
          {
            ...
            for (const p of frame) {
              ...
              if (x2 !== lx || y2 <= ly) {
                ...
                b = 0
                a = 0
                for (let by = 0, by2 = y2; by < opt.size && by2 < h; ++by, ++by2) {
                  for (let bx = 0, bx2 = x2; bx < opt.size && bx2 < w; ++bx, ++bx2) {
   ...
   */
  public static separableConvolution(opt: SeparableConvolutionOptions): MemoryImage {
    const image = opt.image.hasPalette
      ? opt.image.convert({
          numChannels: opt.image.numChannels,
        })
  ...
  public static sketch(opt: SketchOptions): MemoryImage {
    ...
    for (const frame of image.frames) {
      ...
      for (const p of frame) {
        ...
        const h = -topLeft - 2 * top - topRight + bottomLeft + 2 * bottom + bottomRight

        const v = -bottomLeft - 2 * left - topLeft + bottomRight + 2 * right + topRight
  ...
  public static stretchDistortion(opt: StretchDistortionOptions): MemoryImage {
    ...
    for (const frame of image.frames) {
      ...
      for (const p of frame) {
        ...
        ncX = Math.abs(ncX)
        ncY = Math.abs(ncY)
        ncX = (0.5 * ncX + 0.5 * MathUtils.smoothStep(0.25, 0.5, ncX) * ncX) * sX
        ncY = (0.5 * ncY + 0.5 * MathUtils.smoothStep(0.25, 0.5, ncY) * ncY) * sY
        ncX += nCntX
        ncY += nCntY
