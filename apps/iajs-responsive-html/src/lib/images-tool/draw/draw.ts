...
export abstract class Draw {
   ...
   */
  private static calculateCircumference(image: MemoryImage, center: Point, radius: number): Point[] {
    if (
      radius < 0 ||
      center.x + radius < 0 ||
   ...
   */
  private static drawAntialiasCircle(opt: DrawAntialiasCircleOptions): MemoryImage {
    /**
     ...
     */
    const drawPixel4 = (x: number, y: number, dx: number, dy: number, alpha: number): void => {
      // bottom right
   ...
   */
  private static colorDistance(c1: number[], c2: number[], compareAlpha: boolean): number {
    const d1 = c1[0] - c2[0]
    const d2 = c1[1] - c2[1]
    const d3 = c1[2] - c2[2]
    if (compareAlpha) {
  ...
  public static drawPixel(opt: DrawPixelOptions): MemoryImage {
    ...
    const msk = opt.mask?.getPixel(opt.pos.x, opt.pos.y).getChannelNormalized(maskChannel) ?? 1

    let overlayR = opt.filter !== undefined ? opt.color.rNormalized * opt.filter.rNormalized : opt.color.rNormalized
    let overlayG = opt.filter !== undefined ? opt.color.gNormalized * opt.filter.gNormalized : opt.color.gNormalized
    let overlayB = opt.filter !== undefined ? opt.color.bNormalized * opt.filter.bNormalized : opt.color.bNormalized

    const a =
      (opt.color.length < 4 ? 1 : opt.color.aNormalized) *
      (opt.filter === undefined || opt.filter.length < 4 ? 1 : opt.filter.aNormalized)
    ...
    switch (blend) {
      ...
      case BlendMode.dodge:
        {
          ...
          const oR = MathUtils.clamp(
            (overlayR / MathUtils.clamp(overlayA, 0.01, 1)) * MathUtils.step(0, overlayA),
            0,
            0.99,
          )
          const oG = MathUtils.clamp(
            (overlayG / MathUtils.clamp(overlayA, 0.01, 1)) * MathUtils.step(0, overlayA),
            0,
            0.99,
          )
          const oB = MathUtils.clamp(
            (overlayB / MathUtils.clamp(overlayA, 0.01, 1)) * MathUtils.step(0, overlayA),
            0,
            0.99,
          )
      ...
      case BlendMode.overlay:
        if (2 * baseR < baseA) {
          overlayR = 2 * overlayR * baseR + overlayR * (1 - baseA) + baseR * (1 - overlayA)
        } else {
          overlayR =
            overlayA * baseA -
        ...
        if (2 * baseG < baseA) {
          overlayG = 2 * overlayG * baseG + overlayG * (1 - baseA) + baseG * (1 - overlayA)
        } else {
          overlayG =
            overlayA * baseA -
        ...
        if (2 * baseB < baseA) {
          overlayB = 2 * overlayB * baseB + overlayB * (1 - baseA) + baseB * (1 - overlayA)
        } else {
          overlayB =
            overlayA * baseA -
      ...
      case BlendMode.softLight:
        overlayR =
          baseA === 0
            ? 0
            : baseR * (overlayA * (baseR / baseA) + 2 * overlayR * (1 - baseR / baseA)) +
              overlayR * (1 - baseA) +
              baseR * (1 - overlayA)
        ...
        overlayG =
          baseA === 0
            ? 0
            : baseG * (overlayA * (baseG / baseA) + 2 * overlayG * (1 - baseG / baseA)) +
              overlayG * (1 - baseA) +
              baseG * (1 - overlayA)
        ...
        overlayB =
          baseA === 0
            ? 0
            : baseB * (overlayA * (baseB / baseA) + 2 * overlayB * (1 - baseB / baseA)) +
              overlayB * (1 - baseA) +
              baseB * (1 - overlayA)
        break
  ...
  public static fillFlood(opt: FillFloodOptions): MemoryImage {
    ...
    if (threshold > 0) {
      ...
      array = (y: number, x: number) => {
        return visited[y * opt.image.width + x] === 0 && Draw.testPixelLabColorDistance(opt.image, x, y, lab, threshold)
      }
    } else if (!compareAlpha) {
      array = (y: number, x: number) => {
        return visited[y * opt.image.width + x] === 0 && Draw.setAlpha(opt.image.getPixel(x, y), 0) !== srcColor
      }
    } else {
      array = (y: number, x: number) => {
        return visited[y * opt.image.width + x] === 0 && opt.image.getPixel(x, y) !== srcColor
      }
    }
  ...
  public static fillPolygon(opt: FillPolygonOptions): MemoryImage {
    ...
    if (numVertices === 2) {
      return Draw.drawLine({
        image: opt.image,
        line: new Line(opt.vertices[0].x, opt.vertices[0].y, opt.vertices[1].x, opt.vertices[1].y),
        color: opt.color,
        mask: opt.mask,
        maskChannel: maskChannel,
      })
    ...
    const vi = ArrayUtils.generate<number>(numVertices + 1, (i) => (i < numVertices ? i : 0))
  ...
  public static fillRect(opt: FillRectOptions): MemoryImage {
    ...
    if (radius > 0) {
      ...
      while (((it = range.next()), !it.done)) {
        ...
        const m = opt.mask?.getPixel(p.x, p.y).getChannelNormalized(maskChannel) ?? 1
    ...
    if (!alphaBlend || (opt.color.a === opt.color.maxChannelValue && opt.mask === undefined)) {
      const range = opt.image.getRange(xx0, yy0, ww, hh)
      let it: IteratorResult<Pixel> | undefined = undefined
      while (((it = range.next()), !it.done)) {
  ...
  public static fill(opt: FillOptions): MemoryImage {
    ...
    for (const p of opt.image) {
      const maskValue = opt.mask.getPixel(p.x, p.y).getChannelNormalized(maskChannel)
    ...
    if (threshold > 0) {
      ...
      array = (y: number, x: number) => {
        return (
          ...
          (ret[y * opt.image.width + x] !== 0 || Draw.testPixelLabColorDistance(opt.image, x, y, lab, threshold))
        )
      }
    } else if (!compareAlpha) {
      array = (y: number, x: number) => {
        return (
          visited[y * opt.image.width + x] === 0 &&
          (ret[y * opt.image.width + x] !== 0 || Draw.setAlpha(opt.image.getPixel(x, y), 0) !== srcColor)
        )
      }
    } else {
      array = (y: number, x: number) => {
        return (
          visited[y * opt.image.width + x] === 0 &&
          (ret[y * opt.image.width + x] !== 0 || opt.image.getPixel(x, y) !== srcColor)
        )
      }
    }
  ...
  public static compositeImage(opt: CompositeImageOptions): MemoryImage {
    ...
    const dstW = opt.dstW ?? (dst.width < opt.src.width ? dst.width : opt.src.width)
    const dstH = opt.dstH ?? (dst.height < opt.src.height ? dst.height : opt.src.height)
    const blend = opt.blend ?? BlendMode.alpha
    const linearBlend = opt.linearBlend ?? false
    ...
    const dy = srcH / dstH
    const dx = srcW / dstW
    const yCache = Array.from({ length: dstH }, (_, y) => srcY + Math.trunc(y * dy))
    const xCache = Array.from({ length: dstW }, (_, x) => srcX + Math.trunc(x * dx))

    if (blend === BlendMode.direct) {
      Draw.imgDirectComposite(opt.src, dst, dstX, dstY, dstW, dstH, xCache, yCache, maskChannel, opt.mask)
    } else {
      Draw.imgComposite(opt.src, dst, dstX, dstY, dstW, dstH, xCache, yCache, blend, linearBlend, maskChannel, opt.mask)
    }
