...
export class MemoryImageDataInt8 implements MemoryImageData, Iterable<Pixel> {
  ...
  constructor(width: number, height: number, numChannels: number, data?: Int8Array) {
    ...
    this._data = data ?? new Int8Array(this._width * this._height * this._numChannels)
  }
   ...
   */
  public static from(other: MemoryImageDataInt8, skipPixels = false): MemoryImageDataInt8 {
    const data = skipPixels ? new Int8Array(other.data.length) : other.data.slice()
    return new MemoryImageDataInt8(other.width, other.height, other._numChannels, data)
  }
  ...
  public getRange(x: number, y: number, width: number, height: number): Iterator<Pixel> {
    return new PixelRangeIterator(PixelInt8.imageData(this), x, y, width, height)
  }
  ...
  public getColor(r: number, g: number, b: number, a?: number): Color {
    return a === undefined
      ...
      : ColorInt8.rgba(Math.trunc(r), Math.trunc(g), Math.trunc(b), Math.trunc(a))
  }
