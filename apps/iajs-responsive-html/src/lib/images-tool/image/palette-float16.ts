...
export class PaletteFloat16 implements Palette {
   ...
   */
  public setRgba(index: number, r: number, g: number, b: number, a: number): void {
    let _index = index
    _index *= this._numChannels
    this._data[_index] = Float16.doubleToFloat16(r)
    if (this._numChannels > 1) {
  ...
  public get(index: number, channel: number): number {
    return channel < this._numChannels ? Float16.float16ToDouble(this._data[index * this._numChannels + channel]) : 0
  }
