...
export class MemoryImageDataFloat64 implements MemoryImageData, Iterable<Pixel> {
  ...
  constructor(width: number, height: number, numChannels: number, data?: Float64Array) {
    ...
    this._data = data ?? new Float64Array(this._width * this._height * 4 * this._numChannels)
  }
   ...
   */
  public static from(other: MemoryImageDataFloat64, skipPixels = false): MemoryImageDataFloat64 {
    const data = skipPixels ? new Float64Array(other.data.length) : other.data.slice()
    return new MemoryImageDataFloat64(other.width, other.height, other._numChannels, data)
  }
  ...
  public getRange(x: number, y: number, width: number, height: number): Iterator<Pixel> {
    return new PixelRangeIterator(PixelFloat64.imageData(this), x, y, width, height)
  }
  ...
  public getColor(r: number, g: number, b: number, a?: number): Color {
    return a === undefined ? ColorFloat64.rgb(r, g, b) : ColorFloat64.rgba(r, g, b, a)
  }
