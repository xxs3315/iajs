...
export class MemoryImageDataUint1 implements MemoryImageData, Iterable<Pixel> {
   ...
   */
  public static palette(width: number, height: number, palette?: Palette): MemoryImageDataUint1 {
   ...
   * @returns {MemoryImageDataUint1} A new MemoryImageDataUint1 instance.
   */
  public static from(other: MemoryImageDataUint1, skipPixels = false): MemoryImageDataUint1 {
    const data = skipPixels ? new Uint8Array(other.data.length) : other.data.slice()
    const d = new MemoryImageDataUint1(other.width, other.height, other._numChannels, data)
  ...
  public getRange(x: number, y: number, width: number, height: number): Iterator<Pixel> {
    return new PixelRangeIterator(PixelUint1.imageData(this), x, y, width, height)
  }
  ...
  public getColor(r: number, g: number, b: number, a?: number): Color {
    return a === undefined
      ...
      : ColorUint1.rgba(Math.trunc(r), Math.trunc(g), Math.trunc(b), Math.trunc(a))
  }
