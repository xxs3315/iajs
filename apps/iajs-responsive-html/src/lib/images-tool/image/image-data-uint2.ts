...
export class MemoryImageDataUint2 implements MemoryImageData, Iterable<Pixel> {
   ...
   */
  public static palette(width: number, height: number, palette?: Palette): MemoryImageDataUint2 {
   ...
   * @returns {MemoryImageDataUint2} A new MemoryImageDataUint2 instance.
   */
  public static from(other: MemoryImageDataUint2, skipPixels = false): MemoryImageDataUint2 {
    const data = skipPixels ? new Uint8Array(other.data.length) : other.data.slice()
    const d = new MemoryImageDataUint2(other.width, other.height, other._numChannels, data)
  ...
  public getRange(x: number, y: number, width: number, height: number): Iterator<Pixel> {
    return new PixelRangeIterator(PixelUint2.imageData(this), x, y, width, height)
  }
  ...
  public getColor(r: number, g: number, b: number, a?: number): Color {
    return a === undefined
      ...
      : ColorUint2.rgba(Math.trunc(r), Math.trunc(g), Math.trunc(b), Math.trunc(a))
  }
