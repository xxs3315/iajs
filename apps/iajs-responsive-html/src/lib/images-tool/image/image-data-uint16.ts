...
export class MemoryImageDataUint16 implements MemoryImageData, Iterable<Pixel> {
   ...
   * @returns {MemoryImageDataUint16} A new MemoryImageDataUint16 instance.
   */
  public static from(other: MemoryImageDataUint16, skipPixels = false): MemoryImageDataUint16 {
    const data = skipPixels ? new Uint16Array(other.data.length) : other.data.slice()
    const d = new MemoryImageDataUint16(other.width, other.height, other._numChannels, data)
  ...
  public getRange(x: number, y: number, width: number, height: number): Iterator<Pixel> {
    return new PixelRangeIterator(PixelUint16.imageData(this), x, y, width, height)
  }
  ...
  public getColor(r: number, g: number, b: number, a?: number): Color {
    return a === undefined
      ...
      : ColorUint16.rgba(Math.trunc(r), Math.trunc(g), Math.trunc(b), Math.trunc(a))
  }
