...
export class MemoryImageDataInt32 implements MemoryImageData, Iterable<Pixel> {
  ...
  constructor(width: number, height: number, numChannels: number, data?: Int32Array) {
    ...
    this._data = data ?? new Int32Array(this._width * this._height * this._numChannels)
  }
   ...
   */
  public static from(other: MemoryImageDataInt32, skipPixels = false): MemoryImageDataInt32 {
    const data = skipPixels ? new Int32Array(other.data.length) : other.data.slice()
    return new MemoryImageDataInt32(other.width, other.height, other._numChannels, data)
  }
  ...
  public getRange(x: number, y: number, width: number, height: number): Iterator<Pixel> {
    return new PixelRangeIterator(PixelInt32.imageData(this), x, y, width, height)
  }
  ...
  public getColor(r: number, g: number, b: number, a?: number): Color {
    return a === undefined
      ...
      : ColorInt32.rgba(Math.trunc(r), Math.trunc(g), Math.trunc(b), Math.trunc(a))
  }
