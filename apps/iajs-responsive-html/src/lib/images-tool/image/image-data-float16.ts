...
export class MemoryImageDataFloat16 implements MemoryImageData, Iterable<Pixel> {
  ...
  constructor(width: number, height: number, numChannels: number, data?: Uint16Array) {
    ...
    this._data = data ?? new Uint16Array(this._width * this._height * this._numChannels)
  }
   ...
   */
  public static from(other: MemoryImageDataFloat16, skipPixels = false): MemoryImageDataFloat16 {
    const data = skipPixels ? new Uint16Array(other.data.length) : other.data.slice()
    return new MemoryImageDataFloat16(other.width, other.height, other._numChannels, data)
  }
  ...
  public getRange(x: number, y: number, width: number, height: number): Iterator<Pixel> {
    return new PixelRangeIterator(PixelFloat16.imageData(this), x, y, width, height)
  }
  ...
  public getColor(r: number, g: number, b: number, a?: number): Color {
    return a === undefined ? ColorFloat16.rgb(r, g, b) : ColorFloat16.rgba(r, g, b, a)
  }
