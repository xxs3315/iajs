...
export class MemoryImageDataUint32 implements MemoryImageData, Iterable<Pixel> {
  ...
  constructor(width: number, height: number, numChannels: number, data?: Uint32Array) {
    ...
    this._data = data ?? new Uint32Array(this._width * this._height * this._numChannels)
  }
   ...
   */
  public static from(other: MemoryImageDataUint32, skipPixels = false): MemoryImageDataUint32 {
    const data = skipPixels ? new Uint32Array(other.data.length) : other.data.slice()
    return new MemoryImageDataUint32(other.width, other.height, other._numChannels, data)
  }
  ...
  public getRange(x: number, y: number, width: number, height: number): Iterator<Pixel> {
    return new PixelRangeIterator(PixelUint32.imageData(this), x, y, width, height)
  }
  ...
  public getColor(r: number, g: number, b: number, a?: number): Color {
    return a === undefined
      ...
      : ColorUint32.rgba(Math.trunc(r), Math.trunc(g), Math.trunc(b), Math.trunc(a))
  }
