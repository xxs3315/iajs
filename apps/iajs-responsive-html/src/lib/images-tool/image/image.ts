...
export class MemoryImage implements Iterable<Pixel> {
   ...
   */
  public static fromResized(other: MemoryImage, width: number, height: number, skipAnimation = false): MemoryImage {
    const image = new MemoryImage({
      ...
      textData: other._textData !== undefined ? new Map<string, string>(other._textData) : undefined,
    })

    if (other._extraChannels !== undefined) {
      image._extraChannels = new Map<string, MemoryImageData>(other._extraChannels)
    }
   ...
   */
  public static from(other: MemoryImage, skipAnimation = false, skipPixels = false): MemoryImage {
    const image = new MemoryImage()
  ...
  public static fromBytes(opt: MemoryImageFromBytesOptions): MemoryImage {
    ...
    const numChannels =
      opt.numChannels ?? (opt.channelOrder !== undefined ? ChannelOrderLength.get(opt.channelOrder)! : 3)
    ...
    if (numChannels === 1) {
      ...
      if (channelOrder !== ChannelOrder.rgb && channelOrder !== ChannelOrder.bgr) {
    ...
    if (image.data !== undefined) {
      ...
      const rowStride = opt.rowStride ?? getRowStride(opt.width, numChannels, format)
  ...
  private initialize(opt: MemoryImageInitializeOptions): void {
    ...
    const palette =
      opt.palette ??
      (withPalette && this.supportsPalette ? this.createPalette(format, paletteFormat, numChannels) : undefined)
   ...
   */
  private createImageData(width: number, height: number, format: Format, numChannels: number, palette?: Palette): void {
    switch (format) {
      case Format.uint1:
   ...
   */
  private createPalette(format: Format, paletteFormat: Format, numChannels: number): Palette | undefined {
  ...
  public getExtraChannel(name: string): MemoryImageData | undefined {
    return this._extraChannels !== undefined ? this._extraChannels.get(name) : undefined
  }
   ...
   */
  public getRange(x: number, y: number, width: number, height: number): Iterator<Pixel> {
  ...
  public toUint8Array(): Uint8Array {
    return this._data?.toUint8Array() ?? (this.buffer !== undefined ? new Uint8Array(this.buffer) : new Uint8Array())
  }
  ...
  public remapChannels(order: ChannelOrder): void {
    if (this.numChannels === 4) {
      if (order === ChannelOrder.abgr || order === ChannelOrder.argb || order === ChannelOrder.bgra) {
   ...
   */
  public getPixelInterpolate(fx: number, fy: number, interpolation = Interpolation.linear): Color {
  ...
  public getPixelLinear(fx: number, fy: number): Color {
     ...
     */
    const linear = (icc: number, inc: number, icn: number, inn: number): number => {
  ...
  public getPixelCubic(fx: number, fy: number): Color {
     ...
     */
    const cubic = (dx: number, ipp: number, icp: number, inp: number, iap: number): number => {
      return (
        icp +
        0.5 *
          (dx * (-ipp + inp) +
   ...
   */
  public setPixelRgb(x: number, y: number, r: number, g: number, b: number): void {
   ...
   */
  public setPixelRgba(x: number, y: number, r: number, g: number, b: number, a: number): void {
  ...
  public convert(opt: MemoryImageConvertOptions): MemoryImage {
    ...
    */
    ...
    if (
      ...
      ((!withPalette && this.palette === undefined) || (withPalette && this.palette !== undefined))
    ) {
    ...
    for (const frame of this._frames) {
      const newImage = new MemoryImage({
        ...
        textData: frame._textData !== undefined ? new Map<string, string>(frame.textData) : undefined,
      })
