...
export class MemoryImageDataFloat32 implements MemoryImageData, Iterable<Pixel> {
  ...
  constructor(width: number, height: number, numChannels: number, data?: Float32Array) {
    ...
    this._data = data ?? new Float32Array(this._width * this._height * this._numChannels)
  }
   ...
   */
  public static from(other: MemoryImageDataFloat32, skipPixels = false): MemoryImageDataFloat32 {
    const data = skipPixels ? new Float32Array(other.data.length) : other.data.slice()
    return new MemoryImageDataFloat32(other.width, other.height, other._numChannels, data)
  }
  ...
  public getRange(x: number, y: number, width: number, height: number): Iterator<Pixel> {
    return new PixelRangeIterator(PixelFloat32.imageData(this), x, y, width, height)
  }
  ...
  public getColor(r: number, g: number, b: number, a?: number): Color {
    return a === undefined ? ColorFloat32.rgb(r, g, b) : ColorFloat32.rgba(r, g, b, a)
  }
