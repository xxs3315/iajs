...
import { MemoryImageData, MemoryImageDataGetBytesOptions, getImageDataBytes } from './image-data.js'
...
export class MemoryImageDataUint4 implements MemoryImageData, Iterable<Pixel> {
  ...
  constructor(width: number, height: number, numChannels: number, data?: Uint8Array) {
    ...
    this._data = data ?? new Uint8Array(Math.max(this._rowStride * this._height, 1))
  }
   ...
   */
  public static palette(width: number, height: number, palette?: Palette): MemoryImageDataUint4 {
   ...
   * @returns {MemoryImageDataUint4} A new MemoryImageDataUint4 instance.
   */
  public static from(other: MemoryImageDataUint4, skipPixels = false): MemoryImageDataUint4 {
    const data = skipPixels ? new Uint8Array(other.data.length) : other.data.slice()
    const d = new MemoryImageDataUint4(other.width, other.height, other._numChannels, data)
  ...
  public getRange(x: number, y: number, width: number, height: number): Iterator<Pixel> {
    return new PixelRangeIterator(PixelUint4.imageData(this), x, y, width, height)
  }
  ...
  public getColor(r: number, g: number, b: number, a?: number): Color {
    return a === undefined
      ...
      : ColorUint4.rgba(Math.trunc(r), Math.trunc(g), Math.trunc(b), Math.trunc(a))
  }
   ...
   * @param {number} b - The blue component.
   */
  public setPixelRgbSafe(x: number, y: number, r: number, g: number, b: number): void {
   ...
   * @param {number} a - The alpha component.
   */
  public setPixelRgbaSafe(x: number, y: number, r: number, g: number, b: number, a: number): void {
    if (x < 0 || x >= this.width || y < 0 || y >= this.height) {
      return
    }
