...
export class NeuralQuantizer implements Quantizer {
  ...
  private static readonly _initAlpha: number = 1 << NeuralQuantizer._alphaBiasShift

  /** Radius bias shift value */
  private static readonly _radiusBiasShift: number = 8

  /** Radius bias value */
  private static readonly _radiusBias: number = 1 << NeuralQuantizer._radiusBiasShift
  ...
  private static readonly alphaRadiusBias: number = 1 << NeuralQuantizer._alphaRadiusBiasShift
  ...
  private static readonly _betaGamma: number = NeuralQuantizer._beta * NeuralQuantizer._gamma
  ...
  private updateRadiusPower(rad: number, alpha: number): void {
    for (let i = 0; i < rad; i++) {
      this._radiusPower[i] = Math.trunc(alpha * (((rad * rad - i * i) * NeuralQuantizer._radiusBias) / (rad * rad)))
    }
  }
  ...
  private specialFind(b: number, g: number, r: number): number {
    for (let i = 0, p = 0; i < this._specials; i++) {
      if (this._network[p++] === b && this._network[p++] === g && this._network[p++] === r) {
        return i
      }
    }
    return -1
   ...
   * @param {number} r - Red component.
   */
  private alterNeighbors(_: number, rad: number, i: number, b: number, g: number, r: number): void {
    let lo = i - rad
    ...
    while (j < hi || k > lo) {
      ...
      if (j < hi) {
        const p = j * 3
        this._network[p] -= (a * (this._network[p] - b)) / NeuralQuantizer.alphaRadiusBias
  ...
  public getIndexImage(image: MemoryImage): MemoryImage {
    ...
    while ((((imageItRes = imageIt.next()), (targetItRes = targetIt.next())), !imageItRes.done && !targetItRes.done)) {
