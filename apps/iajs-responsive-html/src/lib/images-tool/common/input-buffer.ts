...
export class InputBuffer<T extends TypedArray> {
  ...
  constructor(opt: InputBufferInitOptions<T>) {
    ...
    this._end = opt.length !== undefined ? this._start + opt.length : this._buffer.length
  }
   ...
   */
  public static from<T extends TypedArray>(other: InputBuffer<T>, offset?: number, length?: number): InputBuffer<T> {
    ...
    result._end = length !== undefined ? other.offset + offsetFromOther + length : other._end
    return result
  }
   ...
   */
  public memcpy(start: number, length: number, other: InputBuffer<T> | T, offset: number = 0): void {
   ...
   */
  public subarray(count: number, position?: number, offset = 0): InputBuffer<T> {
