...
export class IfdDirectory {
  ...
  public get userComment(): string | undefined {
    ...
    return data !== undefined ? StringUtils.utf8Decoder.decode(data) : undefined
  }
   ...
   */
  private setRational(tag: number, value: Rational | number[] | TypedArray | unknown): boolean {
    if (value instanceof Rational) {
      this._data.set(tag, IfdRationalValue.from(value))
      return true
    } else if (ArrayUtils.isNumArrayOrTypedArray(value) && (value as []).length >= 2) {
  ...
  public static isArrayOfRationalNumbers(value: unknown): boolean {
    return Array.isArray(value) && value.every((v) => ArrayUtils.isNumArrayOrTypedArray(v) && (v as []).length >= 2)
  }
  ...
  public setValue(
    tag: number | string,
    value: Rational[] | number[] | TypedArray | Rational | IfdValue | number | undefined,
  ): void {
    let _tag: string | number | undefined = tag
    if (typeof _tag === 'string') {
      _tag = ExifTagNameToID.get(_tag)
          ...
          switch (tagType) {
            case IfdValueType.byte:
              if (ArrayUtils.isNumArrayOrTypedArray(value)) {
                this._data.set(_tag, new IfdByteValue(new Uint8Array(value as number[])))
            ...
            case IfdValueType.short:
              if (ArrayUtils.isNumArrayOrTypedArray(value)) {
                this._data.set(_tag, new IfdShortValue(new Uint16Array(value as number[])))
              } else if (typeof value === 'number') {
            ...
            case IfdValueType.long:
              if (ArrayUtils.isNumArrayOrTypedArray(value)) {
                this._data.set(_tag, new IfdLongValue(new Uint32Array(value as number[])))
              } else if (typeof value === 'number') {
            ...
            case IfdValueType.sByte:
              if (ArrayUtils.isNumArrayOrTypedArray(value)) {
                this._data.set(_tag, new IfdSByteValue(new Int8Array(value as number[])))
              } else if (typeof value === 'number') {
            ...
            case IfdValueType.undefined:
              if (ArrayUtils.isNumArrayOrTypedArray(value)) {
                this._data.set(_tag, new IfdUndefinedValue(new Uint8Array(value as number[])))
              }
              break
            case IfdValueType.sShort:
              if (ArrayUtils.isNumArrayOrTypedArray(value)) {
                this._data.set(_tag, new IfdSShortValue(new Int16Array(value as number[])))
              } else if (typeof value === 'number') {
            ...
            case IfdValueType.sLong:
              if (ArrayUtils.isNumArrayOrTypedArray(value)) {
                this._data.set(_tag, new IfdSLongValue(new Int32Array(value as number[])))
              } else if (typeof value === 'number') {
            ...
            case IfdValueType.sRational:
              if (ArrayUtils.isArrayOfRational(value)) {
                this._data.set(_tag, new IfdSRationalValue(value as Rational[]))
              } else if (ArrayUtils.isNumArrayOrTypedArray(value) && (value as []).length >= 2) {
                const r = new Rational((value as number[])[0], (value as number[])[1])
            ...
            case IfdValueType.single:
              if (ArrayUtils.isNumArrayOrTypedArray(value)) {
                this._data.set(_tag, new IfdSingleValue(new Float32Array(value as number[])))
              } else if (typeof value === 'number') {
            ...
            case IfdValueType.double:
              if (ArrayUtils.isNumArrayOrTypedArray(value)) {
                this._data.set(_tag, new IfdDoubleValue(new Float64Array(value as number[])))
              } else if (typeof value === 'number') {
