...
export class ExifData extends IfdContainer {
   ...
   */
  private writeDirectory(out: OutputBuffer, ifd: IfdDirectory, dataOffset: number): number {
    ...
    for (const [tag, value] of ifd.entries) {
      ...
      const tagType = tag === stripOffsetTag && value.type === IfdValueType.undefined ? IfdValueType.long : value.type

      const tagLength = tag === stripOffsetTag && value.type === IfdValueType.undefined ? 1 : value.length
   ...
   * @param {IfdDirectory} ifd - The IFD directory.
   */
  private writeDirectoryLargeValues(out: OutputBuffer, ifd: IfdDirectory): void {
   ...
   */
  private readEntry(block: InputBuffer<Uint8Array>, blockOffset: number): ExifEntry {
  ...
  public write(out: OutputBuffer): void {
    ...
    if (this.directories.get('ifd0') === undefined) this.directories.set('ifd0', new IfdDirectory())
