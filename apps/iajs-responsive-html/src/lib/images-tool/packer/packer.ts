...
import { readUshort, readUint, readUTF8, writeUint, writeUshort, writeUTF8, sizeUTF8 } from './bin.js'
...
const crc = {
   ...
   */
  update: function (c: number, buf: Uint8Array, off: number, len: number): number {
 ...
 */
function _writeHeader(data: Uint8Array, o: number, p: string, obj: FileMetadata, t: number, roff?: number): number {
  const wUi = writeUint
 ...
 */
export function parse(buf: ArrayBuffer, onlyNames: boolean): Record<string, FileMetadata> {
  const rUs = readUshort
  const rUi = readUint
  let o = 0
 ...
 */
export function encode(obj: Record<string, Uint8Array>, noCmpr?: boolean): ArrayBuffer {
