import { Control, FabricObject, controlsUtils, Point, TPointerEvent, Transform, TDegree } from 'fabric'
import { PiBy180, toFixed } from '../math'
...
const getWidthHeight = (object: FabricObject, noFixed = false) => {
  const objScale = object.getObjectScaling()
  const point = object._getTransformedDimensions({
    scaleX: objScale.x,
    scaleY: objScale.y,
  })
  if (!noFixed) {
    point.setX(toFixed(point.x, 2))
    point.setY(toFixed(point.y, 2))
  }
  return point
}
