import { ActiveSelection, Canvas, CanvasEvents, FabricObject, Group, Point, StaticCanvas, util } from 'fabric'
  ...
  private objectMoving({ target }: CanvasEvents['object:moving']) {
    ...
    if (target instanceof ActiveSelection) {
      ...
      target.forEachObject((obj) => {
        const parent = obj.parent as Group
        if (parent) {
          needAddParent.add(parent)
        } else {
          needAddParent.add(this.canvas)
        }
      })
